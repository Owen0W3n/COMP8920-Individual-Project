...
## importing other module for calling their functions
...
class KeyboardStore:
    ...
    def __init__(self):
        ...
        # Try loading saved inventory from file
        if not loaded:
            self._load_default_product()
            self.inventory.save_to_json(self.INVENTORY_FILE)

    def _load_default_product(self):
        #Default product saved inorder to run the program in the first time successfully
    ...
    def view_products_by_category(self):
        #displaying the product by calling the function in inventory module
        print("\nView by category:")
    ...
    def build_keyboard(self):
        print("\n=== BUILD YOUR KEYBOARD ===")
        #create list for storing the selected items 
        selected_items = []


        # loop through required categories
        ...
        # ask user to confirm whether put them into the cart
    ...
    def add_product_to_inventory(self):
        #allow admin user to manage inventory
        added = self.inventory.create_product_viewchoice()
        if added:
            self.inventory.save_to_json(self.INVENTORY_FILE)

    def add_to_cart(self):
        #allow customer user to do shopping activities
        print("\nAdd product to cart:")
        print("1. Keyboard Kit")
        print("2. Keycaps")
        print("3. Switches")
